...
class DCGAN(GAN):
    ...
    def _build_discriminator(self): 
        ...
        discrim.add(layers.Conv2D(self.discrim_num_channels*2,(self.discrim_filter_size,self.discrim_filter_size),strides=(2,2),padding='same',input_shape=discrim_input_shape, kernel_regularizer=tf.keras.regularizers.l2(0.05), activity_regularizer=tf.keras.regularizers.l2(0.05)))
        discrim.add(layers.LeakyReLU())
        discrim.add(layers.Dropout(0.3))

        # Input --> (64,64, discrim_num_channels*2)
        # Output --> (32,32, discrim_num_channels*4)
        discrim.add(layers.Conv2D(self.discrim_num_channels*4,(self.discrim_filter_size,self.discrim_filter_size),strides=(2,2),padding='same',input_shape=discrim_input_shape, kernel_regularizer=tf.keras.regularizers.l2(0.05), activity_regularizer=tf.keras.regularizers.l2(0.05)))
        discrim.add(layers.LeakyReLU())
        discrim.add(layers.Dropout(0.3))

        # Input --> (32,32, discrim_num_channels*4)
        # Output --> (16,16, discrim_num_channels*8)
        discrim.add(layers.Conv2D(self.discrim_num_channels*8,(self.discrim_filter_size,self.discrim_filter_size),strides=(2,2),padding='same',input_shape=discrim_input_shape, kernel_regularizer=tf.keras.regularizers.l2(0.05), activity_regularizer=tf.keras.regularizers.l2(0.05)))
        discrim.add(layers.LeakyReLU())
        discrim.add(layers.Dropout(0.3))

        # Input --> (16,16, discrim_num_channels*8)
        # Output --> (8,8, discrim_num_channels*16)
        discrim.add(layers.Conv2D(self.discrim_num_channels*16,(self.discrim_filter_size,self.discrim_filter_size),strides=(2,2),padding='same',input_shape=discrim_input_shape, kernel_regularizer=tf.keras.regularizers.l2(0.05), activity_regularizer=tf.keras.regularizers.l2(0.05)))
        discrim.add(layers.LeakyReLU())
        discrim.add(layers.Dropout(0.3))

        # Input --> (8,8, discrim_num_channels*16)
        # Output --> (4,4, discrim_num_channels*32)
        discrim.add(layers.Conv2D(self.discrim_num_channels*32,(self.discrim_filter_size,self.discrim_filter_size),strides=(2,2),padding='same',input_shape=discrim_input_shape, kernel_regularizer=tf.keras.regularizers.l2(0.1), activity_regularizer=tf.keras.regularizers.l2(0.1)))
        discrim.add(layers.LeakyReLU())
        discrim.add(layers.Dropout(0.3))
    ...
    def _build_generator(self):
        ...
        # Input --> (128,128,3)
        # Output--> (128,128,3)
        model.add(layers.Conv2DTranspose(int(self.gen_num_channels/4), (self.gen_filter_size,self.gen_filter_size), strides=(1,1), padding='same',use_bias=False))
        model.add(layers.BatchNormalization())
        model.add(layers.LeakyReLU())
        ...
        # Input --> (128,128,3)
        # Output--> (128,128,3)
        model.add(layers.Conv2DTranspose(int(self.gen_num_channels/8), (self.gen_filter_size,self.gen_filter_size), strides=(1,1), padding='same',use_bias=False))
        model.add(layers.BatchNormalization())
        model.add(layers.LeakyReLU())
        ...
        # Input --> (128,128,3)
        # Output--> (128,128,3)
        model.add(layers.Conv2DTranspose(int(self.gen_num_channels/16), (self.gen_filter_size,self.gen_filter_size), strides=(1,1), padding='same',use_bias=False))
        model.add(layers.BatchNormalization())
        model.add(layers.LeakyReLU())
        ...
        # Input --> (128,128,3)
        # Output--> (128,128,3)
        model.add(layers.Conv2DTranspose(int(self.gen_num_channels/32), (self.gen_filter_size,self.gen_filter_size), strides=(1,1), padding='same',use_bias=False))
        model.add(layers.BatchNormalization())
        model.add(layers.LeakyReLU())
    ...
    def discTrain(self, disc_lr, real_images,logger): 
        ...
        with tf.GradientTape(persistent=True) as disc_tape: 
            ...
            logger.log_metric('Discriminator Loss', d_loss)
    ...
    def genTrain(self, gen_lr, logger): 
    ...
    def train_step(self, real_images, gen_lr, disc_lr, gen_train_freq, disc_train_freq, logger):
        ...
        for freq in np.arange(0,gen_train_freq,1):
            gen_imgs = self.genTrain(gen_lr, logger)

        for freq in np.arange(0,disc_train_freq,1):
            self.discTrain(disc_lr, real_images, logger)
